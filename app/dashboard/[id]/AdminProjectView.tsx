...
export default function AdminProjectView({
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: "var(--ds-bg)" }}>
      {/* ── Header ── */}
      <header className="sticky top-0 z-10 px-4 md:px-8 py-3 md:py-5 border-b bg-[#09040F]/80 backdrop-blur-2xl border-white/5">
        <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-6 max-w-[1600px] mx-auto">

          {/* Row 1: back + name + status */}
          <div className="flex items-center gap-3 min-w-0">
            <button
              onClick={() => router.push("/dashboard")}
              className="w-10 h-10 shrink-0 rounded-[1.25rem] flex items-center justify-center bg-white/5 hover:bg-white/10 transition-all border border-white/5"
              title="Retour"
            >
              <ArrowLeft size={18} strokeWidth={2.2} className="text-[var(--ds-text-primary)]" />
            </button>
            {/* Logo desktop only */}
            <Link href="/dashboard" className="hidden md:block transition-transform hover:scale-105">
              <img src="/logo-koko.png" alt="Koko Prod" className="h-8 object-contain" />
            </Link>
            {/* Avatar */}
            <div className="w-9 h-9 md:w-12 md:h-12 rounded-[1.25rem] md:rounded-[1.5rem] flex items-center justify-center shrink-0 font-bold text-white text-sm md:text-base bg-gradient-to-br from-[#8B5CF6] to-[#6D28D9]">
              {project.client_name.charAt(0).toUpperCase()}
            </div>
            <div className="min-w-0 flex-1">
              <h1 className="font-extrabold text-base md:text-xl leading-none text-[var(--ds-text-primary)] tracking-tighter truncate">
                {project.client_name}
              </h1>
              {project.client_email && (
                <p className="text-xs md:text-sm font-medium text-[var(--ds-text-tertiary)] truncate hidden sm:block">
                  {project.client_email}
                </p>
              )}
            </div>
            ...
            <Select value={status} onValueChange={handleStatusChange}>
              <SelectTrigger
                className="w-auto h-8 md:h-10 rounded-full border-0 gap-1.5 px-3 md:px-5 text-[10px] md:text-[11px] font-extrabold uppercase shrink-0"
          ...
          <div className="flex items-center gap-2 md:gap-3 md:ml-auto overflow-x-auto no-scrollbar pb-0.5 md:pb-0">
            <button
              onClick={handleCopyLink}
              className={cn(
                "h-9 md:h-10 px-3 md:px-5 rounded-full flex items-center gap-1.5 md:gap-2 text-[11px] font-extrabold uppercase tracking-widest transition-all border border-white/5 shrink-0",
                copied
                  ? "bg-[var(--ds-mint)] text-[var(--ds-mint-text)]"
            ...
            <a
              ...
              className="h-9 md:h-10 px-3 md:px-5 rounded-full flex items-center gap-1.5 md:gap-2 text-[11px] font-extrabold uppercase tracking-widest transition-colors text-white shrink-0"
            ...
            <button
              ...
              className={cn(
                ...
                deleteConfirm
                  ...
                  : "bg-white/5 text-[var(--ds-text-secondary)] hover:bg-red-500/10 hover:text-red-500"
              )}
            >
              {deleting ? <Loader2 size={13} strokeWidth={2} className="animate-spin" /> : <Trash2 size={13} strokeWidth={2.5} />}
              <span className="hidden sm:inline">{deleting ? "Suppression…" : deleteConfirm ? "Confirmer ?" : "Supprimer"}</span>
            </button>
          </div>
        </div>
      </header>
      ...
      <main className="flex-1 p-3 md:p-8 max-w-[1600px] w-full mx-auto">
        <div className="grid grid-cols-12 gap-4 md:gap-8 items-start">
          <div className="col-span-12 lg:col-span-8">
            <Tabs defaultValue={defaultTab} orientation="vertical" className="w-full flex flex-col md:flex-row md:items-start gap-4 md:gap-8">
              <TabsList className="glass-card flex-row md:flex-col overflow-x-auto no-scrollbar p-2 h-12 md:h-auto w-full md:w-20 shrink-0 gap-2 !bg-white/5 border-white/10">
                {[
                  ...
                  <TabsTrigger
                    key={tab.value}
                    value={tab.value}
                    className="flex flex-col items-center justify-center gap-1.5 rounded-2xl whitespace-nowrap px-3 h-8 md:h-16 md:w-16 transition-all data-[state=active]:bg-[var(--ds-mint)] data-[state=active]:text-[var(--ds-mint-text)] data-[state=active]:shadow-[0_4px_16px_rgba(139,92,246,0.3)]"
                    title={tab.label}
                  >
                    {tab.icon}
